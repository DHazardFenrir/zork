...
#include "Room.h"
#include "Item.h"
#include "Human.h" // Include Human class
...
int Monster::MakeAttack() {
    ...
    auto target = combat_target.lock();
...
void Monster::Tick() {
    if (combat_target.expired()) {
        combat_target.reset();
    }
    else {
        shared_ptr<Room> currentRoom = std::dynamic_pointer_cast<Room>(parent);
        if (currentRoom) {
            for (const auto& child : currentRoom->GetChildren()) {
                auto humanChild = dynamic_pointer_cast<Human>(child);
                if (humanChild && humanChild->GetName() == combat_target.lock()->GetName()) {
                    combat_target = humanChild;
                    break;
                }
            }
        }
        ...
        if (!combat_target.expired()) {
            auto target = combat_target.lock();
            if (dynamic_cast<Human*>(target.get())) {
                MakeAttack();
            }
            else {
                combat_target.reset();
